...
run_test('创建文档片段', function() {
    ...
    return $fragment instanceof \zxf\Dom\Fragments\DocumentFragment;
});
...
run_test('无效选择器异常', function() {
    ...
    try {
        $doc->find('[');
        return false;
    } catch (\zxf\Dom\Exceptions\InvalidSelectorException $e) {
        return true;
    }
});
...
run_test('Errors::handle 处理错误', function() {
    try {
        ...
        \zxf\Dom\Utils\Errors::handle($e);
        return true;
    }
});

run_test('Errors::silence 静默处理错误', function() {
    $result = \zxf\Dom\Utils\Errors::silence(function() {
...
run_test('Errors::setLoggingEnabled 设置日志', function() {
    \zxf\Dom\Utils\Errors::setLoggingEnabled(false);
    return !\zxf\Dom\Utils\Errors::isLoggingEnabled();
});

run_test('Errors::setLogFile 设置日志文件', function() {
    \zxf\Dom\Utils\Errors::setLogFile('/tmp/test.log');
    $logFile = \zxf\Dom\Utils\Errors::getLogFile();
    return $logFile === '/tmp/test.log';
});

run_test('Errors::setErrorHandler 设置自定义处理器', function() {
    $called = false;
    \zxf\Dom\Utils\Errors::setErrorHandler(function() use (&$called) {
        $called = true;
    });
    try {
